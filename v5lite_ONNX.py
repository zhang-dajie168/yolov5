...
class yolov5_lite():
    def __init__(self, model_pb_path="best.onnx", label_list=[], confThreshold=0.5, nmsThreshold=0.5, objThreshold=0.5):
    ...
    def postprocess(self, frame, outs, pad_hw):
        ...
        for i in indices:
            ...
            outs.append(np.array([left, top, left + width, top + height, confidences[i], classIds[i]]))
        # print(np.array(outs))
    ...
    def detect(self, srcimg):
        ...
        for i in range(self.nl):
            ...
            row_ind += length
        outs = self.postprocess(srcimg, outs, (newh, neww, top, left))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str, default='./1.avi', help="image path")
    parser.add_argument('--modelpath', type=str, default='/home/ymt/SL005-YJ001/components/biz/weights/best.onnx',
                        help="onnx filepath")
    ...
    dataset = LoadStreams_onnx("1.avi", img_size=640)
    for path, im0s, vid_cap, s in dataset:
        outs = net.detect(im0s[0].copy())
        print(outs)
