...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5,
                 ...
                 is_addNoise=False, is_changeLight=True, is_cutout=False, is_rotate_img_bbox=True,
...
if __name__ == '__main__':
    ...
    # 读取配置文件
    with open(R'E:\yolov5\config_data.yaml', 'r',encoding='utf-8') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)

        is_open_augement = config['DataAug']['is_open_augement']  # 是否打开数据增强
        is_addNoise = config['DataAug']['is_addNoise']  # 随机模糊
        is_changeLight = config['DataAug']['is_changeLight']  # 调整亮度
        is_cutout = config['DataAug']['is_cutout']  # 裁剪
        is_rotate_img_bbox = config['DataAug']['is_rotate_img_bbox']  # 旋转
        is_crop_img_bboxes = config['DataAug']['is_crop_img_bboxes']  # 复制目标框
        is_shift_pic_bboxes = config['DataAug']['is_shift_pic_bboxes']  # 水平镜像翻转
        is_filp_pic_bboxes = config['DataAug']['is_filp_pic_bboxes']  # 垂直镜像翻转
        need_aug_num = config['DataAug']['need_aug_num']  # 每张图片需要增强的次数
        is_endwidth_dot = config['DataAug']['is_endwidth_dot']  # 文件是否以.bmp结尾

        source_img_path = config['DataAug']['source_img_path']
        source_xml_path = config['DataAug']['source_xml_path']
        save_img_path = config['DataAug']['save_img_path']
        save_xml_path = config['DataAug']['save_xml_path']

    dataAug = DataAugmentForObjectDetection(is_addNoise=is_addNoise, is_changeLight=is_changeLight, is_cutout=is_cutout,
                                            is_rotate_img_bbox=is_rotate_img_bbox,
                                            is_crop_img_bboxes=is_crop_img_bboxes,
                                            is_shift_pic_bboxes=is_shift_pic_bboxes,
                                            is_filp_pic_bboxes=is_filp_pic_bboxes
                                            )  # 数据增强工具类
    ...
    parser.add_argument('--source_img_path', type=str,
                        default=source_img_path)
    parser.add_argument('--source_xml_path', type=str,
                        default=source_xml_path)
